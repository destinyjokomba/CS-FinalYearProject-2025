...
export const partyCodeMap: Record<string, Party> = {
  ...
  "Green": "green",
  "Reform": "reform",
  "Reform UK": "reform",  // make sure both variants work
  "SNP": "snp",
  "Other": "other",
};
// map short codes → display info with logo, slogan, demographics
...
function applyReform(answers: Record<string, string>, score: number): number {
  if (answers.household_income === "under £20,000") score += 6;
  if (answers.education_level === "no qualification") score += 4;
  if (answers.satisfaction_national_government === "very dissatisfied") score += 6;
  if (answers.concern_political_corruption === "very concerned") score += 6;
  if (answers.tax_on_wealthy === "no") score += 6; // pro-rich stance
  if (["restrictive", "very restrictive"].includes(answers.immigration_policy_stance)) score += 8;
  if (["high", "very high"].includes(answers.trust_mainstream_media)) score += 3;
  if (["high", "very high"].includes(answers.trust_public_institutions)) score += 3;
  return score;
}

// Green
function applyGreen(answers: Record<string, string>, score: number): number {
  if (answers.household_income === "£20,000–£40,000") score += 5;
  if (answers.housing_status === "renter" || answers.housing_status === "living with family") score += 4;
  if (answers.satisfaction_national_government === "very dissatisfied") score += 6;
  if (answers.importance_social_issues === "very important") score += 5;
  if (answers.climate_priority === "yes") score += 10;
  if (["low","very low"].includes(answers.trust_mainstream_media)) score += 3;
  if (["low","very low"].includes(answers.trust_public_institutions)) score += 3;
  if (answers.immigration_policy_stance === "more open") score += 4;
  return score;
}
...
function applyLabour(answers: Record<string, string>, score: number): number {
  if (answers.household_income === "£20,000–£40,000") score += 5;
  if (answers.housing_status === "renter" || answers.housing_status === "council housing") score += 4;
  if (["neutral","dissatisfied"].includes(answers.satisfaction_national_government)) score += 3;
  if (answers.importance_social_issues?.includes("important")) score += 4;
  if (["low","very low"].includes(answers.trust_mainstream_media)) score += 3;
  if (["low","very low"].includes(answers.trust_public_institutions)) score += 3;
  if (answers.immigration_policy_stance === "more open") score += 4;
  return score;
...
function applyConservative(answers: Record<string, string>, score: number): number {
  if (answers.household_income === "£60,000-£80,000" || answers.household_income === "£80,000 +") score += 5;
  if (answers.housing_status === "homeowner") score += 4;
  if (answers.satisfaction_national_government?.includes("dissatisfied")) score += 3;
  if (["high","very high"].includes(answers.trust_mainstream_media)) score += 4;
  if (["high","very high"].includes(answers.trust_public_institutions)) score += 4;
  if (answers.immigration_policy_stance === "restrictive") score += 6;
  if (answers.concern_political_corruption === "not concerned") score += 3;
  return score;
}

// Lib Dem
function applyLibDem(answers: Record<string, string>, score: number): number {
  if (answers.household_income === "£40,000–£60,000") score += 4;
  if (["bachelors degree","masters degree","phd or higher"].includes(answers.education_level)) score += 4;
  if (answers.satisfaction_national_government === "neutral") score += 4;
  if (answers.trust_mainstream_media === "medium") score += 3;
  if (answers.trust_public_institutions === "medium") score += 3;
...
function applySNP(answers: Record<string, string>, score: number): number {
  if (answers.region_boost === "Scotland" || answers.constituency_leaning === "snp") score += 12;
  if (answers.satisfaction_national_government?.includes("dissatisfied")) score += 5;
  if (answers.climate_priority === "yes") score += 3;
  if (answers.immigration_policy_stance === "more open") score += 3;
  if (["low","very low"].includes(answers.trust_public_institutions)) score += 3;
  return score;
}
...
function applyOther(answers: Record<string, string>, score: number): number {
  if (answers.constituency_leaning === "other") score += 6;
  if (answers.region_boost === "Wales") score += 5; // Plaid Cymru
  return score;
}
...
export function predictParty(answers: Record<string, string>): PredictionResult {
  ...
  if (topParties.length === 1) {
    ...
    const order: Party[] = ["lab", "reform", "con", "ld", "green", "snp", "other"];
    winner = order.find((p) => topParties.some(([tp]) => tp === p)) as Party;
  }
