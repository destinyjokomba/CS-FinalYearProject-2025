// 🔑 Long names → short codes
export const partyCodeMap = {
    "Labour": "lab",
    "Conservative": "con",
    "Liberal Democrats": "ld",
    "Green": "green",
    "Reform": "reform",
    "Reform UK": "reform", // make sure both variants work
    "SNP": "snp",
    "Other": "other",
};
// map short codes → display info with logo, slogan, demographics
export const partyDisplayMap = {
    lab: {
        name: "Labour",
        color: "#E4003B",
        description: "Centre-left, focused on social equality and welfare spending.",
        logo: "/logos/labour.png",
        slogan: "For the many, not the few",
        demographics: "Younger voters, renters, working-class, public sector workers.",
    },
    con: {
        name: "Conservative",
        color: "#0087DC",
        description: "Centre-right, focused on free markets and traditional values.",
        logo: "/logos/conservative.svg.png",
        slogan: "Strong leadership, secure future",
        demographics: "Older voters, homeowners, higher-income earners, rural areas.",
    },
    reform: {
        name: "Reform UK",
        color: "#12B6CF",
        description: "Right-wing, anti-establishment and focused on immigration reform.",
        logo: "/logos/reform.svg.png",
        slogan: "Let’s take back control",
        demographics: "Disaffected Conservative voters, anti-EU, older men, working-class.",
    },
    ld: {
        name: "Liberal Democrats",
        color: "#FAA61A",
        description: "Centrist, liberal policies, pro-Europe and civil liberties.",
        logo: "/logos/libdem.png",
        slogan: "Demand better",
        demographics: "Educated professionals, pro-EU, suburban and southern England voters.",
    },
    green: {
        name: "Green",
        color: "#6AB023",
        description: "Environmentalist, progressive on social and climate issues.",
        logo: "/logos/green.svg.png",
        slogan: "People. Planet. Politics.",
        demographics: "Students, urban professionals, climate activists, progressive younger voters.",
    },
    snp: {
        name: "SNP",
        color: "#FDF38E",
        description: "Scottish nationalist, centre-left, pro-independence.",
        logo: "/logos/snp.svg.png",
        slogan: "Stronger for Scotland",
        demographics: "Scottish voters, pro-independence, younger progressive voters.",
    },
    other: {
        name: "Other",
        color: "#888888",
        description: "Smaller regional or independent parties.",
        logo: "/logos/other.png",
        slogan: "Local voices matter",
        demographics: "Niche or regional communities, independents.",
    },
};
// Balanced starting scores
const baseScores = {
    lab: 20,
    con: 18,
    reform: 15,
    ld: 12,
    green: 12,
    snp: 10,
    other: 5,
};
// ───────────────────────────────
// Party-specific scoring functions
// ───────────────────────────────
// Reform UK
function applyReform(answers, score) {
    if (answers.household_income === "under £20,000")
        score += 6;
    if (answers.education_level === "no qualification")
        score += 4;
    if (answers.satisfaction_national_government === "very dissatisfied")
        score += 6;
    if (answers.concern_political_corruption === "very concerned")
        score += 6;
    if (answers.tax_on_wealthy === "no")
        score += 6; // pro-rich stance
    if (["restrictive", "very restrictive"].includes(answers.immigration_policy_stance))
        score += 8;
    if (["high", "very high"].includes(answers.trust_mainstream_media))
        score += 3;
    if (["high", "very high"].includes(answers.trust_public_institutions))
        score += 3;
    return score;
}
// Green
function applyGreen(answers, score) {
    if (answers.household_income === "£20,000–£40,000")
        score += 5;
    if (answers.housing_status === "renter" || answers.housing_status === "living with family")
        score += 4;
    if (answers.satisfaction_national_government === "very dissatisfied")
        score += 6;
    if (answers.importance_social_issues === "very important")
        score += 5;
    if (answers.climate_priority === "yes")
        score += 10;
    if (["low", "very low"].includes(answers.trust_mainstream_media))
        score += 3;
    if (["low", "very low"].includes(answers.trust_public_institutions))
        score += 3;
    if (answers.immigration_policy_stance === "more open")
        score += 4;
    return score;
}
// Labour
function applyLabour(answers, score) {
    if (answers.household_income === "£20,000–£40,000")
        score += 5;
    if (answers.housing_status === "renter" || answers.housing_status === "council housing")
        score += 4;
    if (["neutral", "dissatisfied"].includes(answers.satisfaction_national_government))
        score += 3;
    if (answers.importance_social_issues?.includes("important"))
        score += 4;
    if (["low", "very low"].includes(answers.trust_mainstream_media))
        score += 3;
    if (["low", "very low"].includes(answers.trust_public_institutions))
        score += 3;
    if (answers.immigration_policy_stance === "more open")
        score += 4;
    return score;
}
// Conservative
function applyConservative(answers, score) {
    if (answers.household_income === "£60,000-£80,000" || answers.household_income === "£80,000 +")
        score += 5;
    if (answers.housing_status === "homeowner")
        score += 4;
    if (answers.satisfaction_national_government?.includes("dissatisfied"))
        score += 3;
    if (["high", "very high"].includes(answers.trust_mainstream_media))
        score += 4;
    if (["high", "very high"].includes(answers.trust_public_institutions))
        score += 4;
    if (answers.immigration_policy_stance === "restrictive")
        score += 6;
    if (answers.concern_political_corruption === "not concerned")
        score += 3;
    return score;
}
// Lib Dem
function applyLibDem(answers, score) {
    if (answers.household_income === "£40,000–£60,000")
        score += 4;
    if (["bachelors degree", "masters degree", "phd or higher"].includes(answers.education_level))
        score += 4;
    if (answers.satisfaction_national_government === "neutral")
        score += 4;
    if (answers.trust_mainstream_media === "medium")
        score += 3;
    if (answers.trust_public_institutions === "medium")
        score += 3;
    if (answers.immigration_policy_stance === "more open")
        score += 3;
    return score;
}
// SNP
function applySNP(answers, score) {
    if (answers.region_boost === "Scotland" || answers.constituency_leaning === "snp")
        score += 12;
    if (answers.satisfaction_national_government?.includes("dissatisfied"))
        score += 5;
    if (answers.climate_priority === "yes")
        score += 3;
    if (answers.immigration_policy_stance === "more open")
        score += 3;
    if (["low", "very low"].includes(answers.trust_public_institutions))
        score += 3;
    return score;
}
// Other
function applyOther(answers, score) {
    if (answers.constituency_leaning === "other")
        score += 6;
    if (answers.region_boost === "Wales")
        score += 5; // Plaid Cymru
    return score;
}
// ───────────────────────────────
// Main predictor
// ───────────────────────────────
export function predictParty(answers) {
    const scores = { ...baseScores };
    scores.lab = applyLabour(answers, scores.lab);
    scores.con = applyConservative(answers, scores.con);
    scores.reform = applyReform(answers, scores.reform);
    scores.ld = applyLibDem(answers, scores.ld);
    scores.green = applyGreen(answers, scores.green);
    scores.snp = applySNP(answers, scores.snp);
    scores.other = applyOther(answers, scores.other);
    // Pick winner
    const sorted = Object.entries(scores).sort((a, b) => b[1] - a[1]);
    const topScore = sorted[0][1];
    const topParties = sorted.filter(([, score]) => score === topScore);
    let winner;
    if (topParties.length === 1) {
        winner = topParties[0][0];
    }
    else {
        const order = ["lab", "reform", "con", "ld", "green", "snp", "other"];
        winner = order.find((p) => topParties.some(([tp]) => tp === p));
    }
    // Normalised probabilities
    const total = Object.values(scores).reduce((a, b) => a + b, 0) || 1;
    const probabilities = Object.fromEntries(Object.entries(scores).map(([party, score]) => [
        party,
        parseFloat(((score / total) * 100).toFixed(2)),
    ]));
    return { winner, probabilities };
}
